    ...
    internal class VehiculoController
    {
        ...
        public static Vehiculo InsertarVehiculo(Vehiculo vehiculo)
        {
            try
            {
                ...
                {
                    var consulta = "INSERT INTO Vehiculos (vehiculo_id, marca, modelo, año, placa) VALUES (@IdVehiculo, @Marca, @Modelo, @Año, @Placa)";
                    using (var comando = new SqlCommand(consulta, conexion))
                    {
                        ...
                        using (var lector = comando.ExecuteReader())
                        {
                            if (lector.Read())
                            {
                                return new Vehiculo
                                {
                                    IdVehiculo = lector["vehiculo_id"].ToString(),
        ...
        public static string ActualizarVehiculo(Vehiculo vehiculo)
        {
            try
            {
                using (var conexion = Conexion.GetConnection())
                {
                    var consulta = "UPDATE Vehiculos SET marca = @Marca, modelo = @Modelo, año = @Año, placa = @Placa WHERE vehiculo_id = @IdVehiculo";
                    using (var comando = new SqlCommand(consulta, conexion))
                    {
                        comando.Parameters.AddWithValue("@IdVehiculo", vehiculo.IdVehiculo);
                        comando.Parameters.AddWithValue("@Marca", vehiculo.Marca);
        ...
        public static string EliminarVehiculo(string idVehiculo)
        {
            try
            {
                using (var conexion = Conexion.GetConnection())
                {
                    var consulta = "DELETE FROM Vehiculos WHERE vehiculo_id = @IdVehiculo";
                    using (var comando = new SqlCommand(consulta, conexion))
                    {
        ...
        public static Vehiculo ObtenerVehiculoPorId(string idVehiculo)
        {
            try
            {
                using (var conexion = Conexion.GetConnection())
                {
                    var consulta = "SELECT * FROM Vehiculos WHERE vehiculo_id = @IdVehiculo";
                    using (var comando = new SqlCommand(consulta, conexion))
                    {
                        comando.Parameters.AddWithValue("@IdVehiculo", idVehiculo);
